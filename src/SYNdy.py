...
# Remove duplicates in time_numeric by averaging data at duplicate timestamps
...
time_unique = time_df.groupby('time', as_index=False).mean()
time_numeric = time_unique['time']
cold_valve = time_unique['cold_valve']
hot_valve = time_unique['hot_valve']
var2 = time_unique['var2']
flow_rate = time_unique['flow_rate']
mixed_temp = time_unique['mixed_temp']

if len(time_unique) < len(time_df):
    print("Duplicate timestamps detected. Averaged data for duplicate times.")
else:
    print("No duplicate timestamps found.")

# Check for NaN after averaging
print("Checking for NaN values after averaging duplicates...")
...
# Smooth data to reduce noise impact on derivatives
window_size = 5
...
if len(t) < 50:
    ...
    print("Consider revisiting the timestamp averaging or resampling step to retain more data points.")

# Define SINDy model with reduced complexity
print("Setting up SINDy model with reduced complexity...")
library = PolynomialLibrary(degree=1, include_interaction=False)  # Linear terms only
...
# Fit the SINDy model to the data with error handling
print("Fitting SINDy model to data...")
try:
    model.fit(X, t=t, u=U, quiet=False)
except Exception as e:
...
# Simulate the model for validation
...
# Debug shapes of X and X_pred
print("Shape of actual data X:", X.shape)
print("Shape of predicted data X_pred:", X_pred.shape)

# Handle dimension mismatch by trimming to the shorter length
n_points = min(X.shape[0], X_pred.shape[0])
X_trimmed = X[:n_points, :]
X_pred_trimmed = X_pred[:n_points, :]
t_trimmed = t[:n_points]

# Compute fit percentages
err_flow = X_trimmed[:, 0] - X_pred_trimmed[:, 0]
fit_flow = 100 * (1 - np.linalg.norm(err_flow) / np.linalg.norm(X_trimmed[:, 0] - np.mean(X_trimmed[:, 0])))
err_temp = X_trimmed[:, 1] - X_pred_trimmed[:, 1]
fit_temp = 100 * (1 - np.linalg.norm(err_temp) / np.linalg.norm(X_trimmed[:, 1] - np.mean(X_trimmed[:, 1])))
print(f"SINDy Fit for Flow_Rate: {fit_flow:.2f}%")
print(f"SINDy Fit for Mixed_Temperature: {fit_temp:.2f}%")

# Plot results for validation (denormalize for readability if needed)
plt.figure(figsize=(10, 8))

plt.subplot(2, 1, 1)
plt.plot(t_trimmed, X_trimmed[:, 0] * flow_rate_std + flow_rate_mean, 'b-', label='Actual Flow_Rate')
plt.plot(t_trimmed, X_pred_trimmed[:, 0] * flow_rate_std + flow_rate_mean, 'r--', label='Predicted Flow_Rate (SINDy)')
...
plt.subplot(2, 1, 2)
plt.plot(t_trimmed, X_trimmed[:, 1] * mixed_temp_std + mixed_temp_mean, 'b-', label='Actual Mixed_Temperature')
plt.plot(t_trimmed, X_pred_trimmed[:, 1] * mixed_temp_std + mixed_temp_mean, 'r--', label='Predicted Mixed_Temperature (SINDy)')
